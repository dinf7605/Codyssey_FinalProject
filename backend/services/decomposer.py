...
LLM_TIMEOUT_SECONDS = 20  # AI기능명세 6
MAX_TOKENS = 4096
...
def decompose_goal(
    ...
    if client is None:
        ...
        if not api_key:
            return DecomposeResult(
                ...
                source="template",
                message="API 키가 없어 표준 커리큘럼으로 시작합니다.",
            )
        try:
            import anthropic
            ...
            return DecomposeResult(
                units=template_units(goal_title),
                source="template",
                message=f"AI 호출을 준비하지 못했습니다: {exc}",
            )
    ...
    for _ in range(MAX_TOOL_ITERATIONS):
        try:
            ...
            return DecomposeResult(
                units=template_units(goal_title),
                source="template",
                message=f"AI 응답을 받지 못해 기본 계획으로 시작합니다. ({type(exc).__name__})",
    ...
    retry = _retry_once(client, model, messages, tool_calls)
    ...
    return DecomposeResult(
        units=template_units(goal_title),
        source="template",
        message="자동 생성에 실패해 기본 계획으로 시작합니다.",
        tool_calls=tool_calls,
    )


def _retry_once(client, model: str, messages: list[dict], tool_calls: int):
